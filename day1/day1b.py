...
def solve(path):
    ...
    lines = np.array([ints(line) for line in lines])
    
    sorted_col1 = np.sort(lines[:, 0])
    sorted_col2 = np.sort(lines[:, 1])

    for i in sorted_col1:
        if i not in sorted_col2:
            continue
        # get count of i in sorted_col2
        ans += np.count_nonzero(sorted_col2 == i) * i
